...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: boxy.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: boxyd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeboxyamount=<n> " + _("Keep N Boxy anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Boxy is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Boxy is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Boxy version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Boxy server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Boxy") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Boxy to complete") << "\n";
    ...
    nAnonymizeboxyAmount = GetArg("-anonymizeboxyamount", 0);
    if(nAnonymizeboxyAmount > 999999) nAnonymizeboxyAmount = 999999;
    if(nAnonymizeboxyAmount < 2) nAnonymizeboxyAmount = 2;
    ...
    LogPrintf("Anonymize Boxy Amount %d\n", nAnonymizeboxyAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BOXY+1000 == (.1BOXY+100)*10
       10BOXY+10000 == (1BOXY+1000)*10
    */
