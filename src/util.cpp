...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Boxy
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Boxy
    // Mac: ~/Library/Application Support/Boxy
    // Unix: ~/.boxy
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Boxy";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "boxy.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "boxyd.pid"));
