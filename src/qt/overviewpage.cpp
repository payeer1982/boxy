...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeboxyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeboxyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeboxyAmount = strAnonymizeboxyAmount.remove(strAnonymizeboxyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeboxyAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeboxyAmount*COIN) nMaxToAnonymize = nAnonymizeboxyAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeboxyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeboxyAmount));
        strAnonymizeboxyAmount = strAnonymizeboxyAmount.remove(strAnonymizeboxyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeboxyAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
