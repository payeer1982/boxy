...
{
    resize(900, 520);
    setWindowTitle(tr("BoxyCore") + " - " + tr("Wallet"));
...
#endif
    setObjectName("boxy");
    setStyleSheet("#boxy { background-color: #ffffff;  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
    ...
    statusBar()->setStyleSheet("#statusBar { color: #ffffff; background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #161931, stop: 1 #404040);  }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Boxy address"));
    ...
    mktAction = new QAction(QIcon(":/icons/marketplace"), "Community", this);
    mktAction->setCheckable(true);
    mktAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(mktAction);
    ...
    connect(mktAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(mktAction, SIGNAL(triggered()), this, SLOT(gotomktPage()));

    quitAction = new QAction(tr("E&xit"), this);
    quitAction->setToolTip(tr("Quit application"));
    quitAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_Q));
    quitAction->setMenuRole(QAction::QuitRole);
    pioAction = new QAction(tr("&Boxy Website"), this);
	pioAction->setToolTip(tr("Visit the Official Boxy website"));
    bitcointalkAction = new QAction(tr("&Bitcointalk Discussion"), this);
	bitcointalkAction->setToolTip(tr("Visit the Bitcointalk Boxy discussion thread"));
    chainAction = new QAction(tr("&Boxy Discord"), this);
    chainAction->setToolTip(tr("Join Boxy Discord"));
    explorerAction = new QAction(tr("&Boxy Explorer"), this);
	explorerAction->setToolTip(tr("Boxy Explorer"));	
    aboutAction = new QAction(tr("&About Boxy"), this);
    aboutAction->setToolTip(tr("Show information about Boxy"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Boxy"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Boxy client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Boxy client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Boxy network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Boxy address or malformed URI parameters."));
    }
...
{
    QDesktopServices::openUrl(QUrl("http://www.boxycoin.live/community/"));
}

void BitcoinGUI::bitcointalkClicked()
{
    QDesktopServices::openUrl(QUrl("http://www.boxycoin.live/bct/"));
}

void BitcoinGUI::chainClicked()
{
    QDesktopServices::openUrl(QUrl("http://www.boxycoin.live/discord/"));
}
void BitcoinGUI::explorerClicked()
{
    QDesktopServices::openUrl(QUrl("http://boxy.blockxplorer.info"));
}
