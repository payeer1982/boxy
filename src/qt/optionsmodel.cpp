...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeboxyAmount"))
        settings.setValue("nAnonymizeboxyAmount", 1000);
    nAnonymizeboxyAmount = settings.value("nAnonymizeboxyAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeboxyAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeboxyAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeboxyAmount:
            return QVariant(nAnonymizeboxyAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeboxyAmount:
            nAnonymizeboxyAmount = value.toInt();
            settings.setValue("nAnonymizeboxyAmount", nAnonymizeboxyAmount);
            emit AnonymizeboxyAmountChanged(nAnonymizeboxyAmount);
            break;
        default:
            break;
        }
